...
| `buttonon()`    | Brain 14 sprites       | Called if the sprite has a [sp_brain()](../functions/sp-brain.md) of 14 and Dink walks on top of its hardbox.                                                                                               |
| `buttonoff()`   | Brain 14 sprites       | Called if the sprite has a [sp_brain()](../functions/sp-brain.md) of 14 and Dink walks off top of its hardbox.                                                                                              |
| `click()`   | Brain 14 sprites       | Called if the sprite has a [sp_brain()](../functions/sp-brain.md) of 14 and the sprite is clicked while in cursor mode.                                                                                           |
| `hit()`         | All sprites    | When the sprite is hit by another sprite.                                                                                                                                       |
...
| `die()`         | Enemy, pig, duck & people sprites    | When the sprite dies in combat. Killing a sprite with [sp_kill()](../functions/sp-kill.md) or [sp_active()](../functions/sp-active.md) will not trigger the die procedure. When Dink dies, the die procedure is run from `dinfo.c`. |
| `duckdie()`       | Duck sprites | When a sprite with brain 3 dies. Killing a sprite with [sp_kill()](../functions/sp-kill.md) or [sp_active()](../functions/sp-active.md) will not trigger the duckdie procedure.       
